...
def twoSum1(nums, target):
    # create a dict to hold potential compliments and their indicies
    possibilities = {}
    # loop through the indicies of the numbers
    for i in range(len(nums)):
        # define the complement as the difference between the target value and the current value in the loop
        compliment = target - nums[i]
        # if the compliment key is in the possibilites dict, return the key of the compliment, which would be compliment's index and the current index of the loop
